...
3. Cooldown/ARM/прозорец състоянието (за да не спамва един и същ сигнал постоянно) се пази в KV между извикванията, по символ. Освен собствените cooldown-и на MM/MM x25/WARMING/MMOSC, има и общ филтър (`tagCanFire`/`SIGNAL_REPEAT_COOLDOWN_MIN` = 20 мин) върху ВСИЧКИ тагове — детекторите без собствен cooldown (FLUSH/BASE/DISTRIBUTION/SQUEEZE/DUMP SQUEEZE/SHIFT/SHIFT▼/IMPULSE/IMPULSE+ATR/CONFIRMED/EARLY BUILD-UP) иначе препращат идентично известие на всяко 5-мин cron изпълнение, докато условието на свещта остане вярно. `longHits`/`shortHits` (виж по-долу) се броят СЛЕД този филтър, само от сигналите, които реално влизат в текущото известие — стар сигнал, потиснат от cooldown, не влияе на %/посоката.
4. Ако нещо ново се е задействало (и не е било пратено за същата монета през последните 20 мин), праща едно консолидирано WhatsApp съобщение с едно от три ясни състояния:
   - **A) Ясна посока + достатъчно потвърждения** (мнозинството сигнали ≥ `MIN_TP_CONFIRMATION_HITS` = 2): `📍 Посока`, `📊 Сигнали: LONG X% / SHORT Y%`, `✅ Потвърждения: N LONG / M SHORT`, после `🎯 TP1-5`.
   - **B) Ясна посока, но под прага**: същия хедър, но вместо TP — `⚠️ Само N потвърждение — TP не се показва` (напр. 1 LONG / 0 SHORT).
   - **C) Смесени сигнали** (мнозинството не покрива `DIRECTION_CONFIDENCE_RATIO` = 4:1 спрямо малцинството): `⚠️ СМЕСЕНИ СИГНАЛИ` + %-но разпределение + брой потвърждения, без TP.
...
**Посока и увереност** (`computeDirectionConfidence` в `worker.js`): всеки задействал се сигнал носи собствена посока (`fired.push({ label, direction })` в `scanSymbolSignals` — не се извежда чрез парсене на текста на label-а). `longHits`/`shortHits` се броят само от сигналите, които реално влизат в текущото известие (след `tagCanFire` cooldown филтъра). Понеже детекторите са напълно независими (различни timeframes/логика), е възможно в една обиколка да се задействат и в двете посоки едновременно за една и съща монета.

Две отделни условия решават какво се показва:
- **Посока се показва** при мнозинство поне 4:1 (`ratioOK` — напр. 4 срещу 1, 8 срещу 2, или 1 срещу 0). При по-слабо мнозинство (напр. 3:2, 2:1, 3:1) известието показва `⚠️ СМЕСЕНИ СИГНАЛИ` вместо подвеждащо уверена посока.
- **TP1-5 се показва** само ако мнозинството достига И `MIN_TP_CONFIRMATION_HITS` = 2 (`enoughHits`) — т.е. 1 срещу 0 показва ясна посока, но НЕ показва TP (`⚠️ Само 1 потвърждение — TP не се показва`), докато 2 срещу 0 вече показва TP. Целта е "1 от 1 сигнал LONG" да не изглежда толкова уверено, колкото "4 от 5".

Процентите (`Сигнали: LONG X% / SHORT Y%`) винаги значат "дял от гласувалите в тази обиколка сигнали", никога вероятност за успех на сделката — никъде не пише "100% увереност".
